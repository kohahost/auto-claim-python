...
# Ganti dengan token bot dan chat ID Anda yang didapat dari Langkah 1
...
def send_telegram_notification(message):
    ...
    # Menggunakan MarkdownV2 untuk format teks (bold, code, dll)
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"[ERROR TELEGRAM] Gagal mengirim notifikasi: {response.json()}")
    except Exception as e:
        print(f"[ERROR TELEGRAM] Terjadi kesalahan saat mengirim notifikasi: {e}")
...
notification_lock = threading.Lock() # <-- DITAMBAHKAN
SPAM_SUCCESS_NOTIFIED = False # <-- DITAMBAHKAN
...
def claim_and_send(i, seq):
    global SEQUENCE, ACCOUNT, SPAM_SUCCESS_NOTIFIED # <-- DITAMBAHKAN
    ...
    try:
        ...
        # Kirim notifikasi HANYA untuk keberhasilan spam pertama
        with notification_lock:
            if not SPAM_SUCCESS_NOTIFIED:
                message = (f"✅ *TRANSAKSI SPAM BERHASIL (No. {i})*\n\n"
                           f"Amount: {UNLOCK_BALANCE} Pi\n"
                           f"Hash: `{response['hash']}`")
                send_telegram_notification(message)
                SPAM_SUCCESS_NOTIFIED = True

    except Exception as e:
        print(f"[ERROR] Transaction -{i}- Error: {e}")
        # Kita tidak mengirim notif untuk error spam agar tidak banjir pesan
...
if UNLOCKS and hasattr(countdown, "tx_prepared"):
    try:
        ...
        message_sukses = (f"✅ *TRANSAKSI UTAMA BERHASIL*\n\n"
                          f"Amount: {UNLOCK_BALANCE} Pi\n"
                          f"Hash: `{response['hash']}`")
        send_telegram_notification(message_sukses)
        ...
        message_gagal = (f"❌ *TRANSAKSI UTAMA GAGAL*\n\n"
                         f"Error: `{str(e)}`")
